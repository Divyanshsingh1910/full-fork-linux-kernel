...
void *forker_thread(void *arg) {
    int thread_id = *(int *)arg;
    printf("Forker thread started (thread ID: %d)\n", thread_id);
    
	int pid = fork();
	if(!pid){
		printf("Child of the forker thread is running\n");
	}
	else{
		printf("Parent of the forker thread is running\n");
	}

	printf("Forker thread exiting (thread ID: %d)\n", thread_id);
    return NULL;
}
...
int main() {
    ...
    pthread_create(&threads[0], NULL, forker_thread, (void *)&thread_ids[0]);
