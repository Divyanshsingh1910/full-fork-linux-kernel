...
void *thread_three(void *arg) {
	...
	int pid = fork();
	if(!pid){
		printf("Child of thread-3 has started\n");
		long long result = fibonacci(n);
	}
	else{
		wait(NULL);
		printf("This is parent of thread-3, do you copy?\n");
	}
...
int main() {
    ...
    pthread_create(&threads[0], NULL, thread_three, (void *)&thread_ids[0]);
	

    pthread_create(&threads[1], NULL, thread_two, (void *)&thread_ids[1]);

	syscall(548);
    pthread_create(&threads[2], NULL, thread_one, (void *)&thread_ids[2]);
