...
void my_syscall_handler(struct pt_regs *regs, int sysnr)
{
	...
	clone:
		...
		times[5] = ktime_get_real_ns();

		printk(KERN_INFO "TIME_INFO: %llu %llu %llu %llu %llu %llu", times[0], times[1], times[2], times[3], times[4], times[5]);
	}
}
